...
- **Password reset:** `POST /auth/forgot-password` is opaque against account enumeration — it always returns `202` with the same message (and is padded to a minimum response duration) whether or not the email exists. Reset tokens are single-use, expiry-gated (60 min), and stored **only as SHA-256 digests** (never raw). One active reset link per account (a new request invalidates any prior unredeemed token). `POST /auth/reset-password` returns a generic `401` for invalid/expired/used tokens (no token-guessing feedback; redemption locks the token row so concurrent replays cannot both succeed) and, on success, revokes all refresh tokens and invalidates the user cache — sessions must re-authenticate once their (≤ 15 min) access tokens expire. Both endpoints are rate-limited (`forgot-password` 5/min, `reset-password` 10/min).
